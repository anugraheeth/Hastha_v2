...
const Edudetails = () => {
  ...
  const getData = async() =>{
      try {
        ...
        setLoading(true);
        const response = await get('api/auth/edudetails');
  ...
  if(users!=null){
    filteredUsers = users.filter(
      (user) =>
        user.name_of_institution.toLowerCase().includes(searchTerm.toLowerCase()) ||
        user.head_of_institution.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }
  ...
  const handleDownloadCSV = () => {
    const csvData = [
      ["Building No", "Institution Name  ", "Name of Head", "Contact","No of Students"],
      ...filteredUsers.map((user) => [user.building_number, user.name_of_institution, user.head_of_institution, user.phone_num,user.no_of_students]),
    ];
  ...
  const handleDownloadPDF = () => {
    ...
    const tableData = filteredUsers.map((user) => [
      ...
      user.head_of_institution, 
      user.phone_num,
      user.no_of_students
    ]);
    doc.autoTable({
      head: [["Building No", "Institution Name  ", "Name of Head", "Contact","No of Students"]],
      body: tableData,
    });
    doc.save("users.pdf");
  };
  ...
  <user>
    ...
    <Institution_Name>${user.name_of_institution}</Institution_Name>
    <Name_of_Head>${user.head_of_institution}</Name_of_Head>
    <Contact>${user.phone_num}</Contact>
    <No_of_Students>${user.no_of_students}</No_of_Students>
  </user>`
      )
      .join("");
        ...
        <div className="flex-1 flex flex-col">
          <header className="w-full flex items-center justify-between bg-white shadow-md p-4">
            <div className="flex items-center">
              ...
              <h2 className="ml-4 text-xl font-semibold text-gray-700">EDUCATIONAL DETAILS</h2>
            </div>
              ...
              <div className="overflow-x-auto bg-white rounded-lg shadow">
                <table className="min-w-full table-auto">
                  <thead className="bg-gray-200">
                    <tr>
                      <th className="p-4 text-sm text-gray-500 text-start">Building No</th>
                      <th className="p-4 text-sm text-gray-500 text-start">Name of Institution</th>
                      <th className="p-4 text-sm text-gray-500 text-start">Name of Head</th>
                      <th className="p-4 text-sm text-gray-500 text-start">Contact Number</th>
                      <th className="p-4 text-sm text-gray-500 text-start">Number of Students</th>
                    </tr>
                  </thead>
                  <tbody >
                    {currentUsers.map((user) => (
                      <tr key={user.id} className="border-b hover:bg-gray-100">
                        ...
                        <td className="p-4">{user.name_of_institution}</td>
                        <td className="p-4">{user.head_of_institution}</td>
                        <td className="p-4">{user.phone_num}</td>
                        <td className="p-4">{user.no_of_students}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
