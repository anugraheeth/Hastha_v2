...
const Inddetails = () => {
  ...
  const getData = async() =>{
      try {
        ...
        setLoading(true);
        const response = await get('api/auth/inddetails');
  ...
  if(users!=null){
    filteredUsers = users.filter(
      (user) =>
        user.name_of_industry.toLowerCase().includes(searchTerm.toLowerCase()) ||
        user.name_of_head.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }
  ...
  const handleDownloadCSV = () => {
    const csvData = [
      ["Name", "Position", "Status", "Email"],
      ...filteredUsers.map((user) => [user.name, user.position, user.status, user.email]),
    ];
  ...
  const handleDownloadPDF = () => {
    ...
    doc.autoTable({
      head: [["Name", "Position", "Status", "Email"]],
      body: tableData,
    });
    doc.save("users.pdf");
  };
        ...
        <div className="flex-1 flex flex-col">
          <header className="w-full flex items-center justify-between bg-white shadow-md p-4">
            <div className="flex items-center">
              ...
              <h2 className="ml-4 text-xl font-semibold text-gray-700">INDUSTRIAL DETAILS</h2>
            </div>
                ...
                <table className="min-w-full table-auto">
                  <thead className="bg-gray-200">
                    <tr>
                      ...
                      <th className="p-4 text-sm text-gray-500 text-start">Name of Industry</th>
                      <th className="p-4 text-sm text-gray-500 text-start">Name of Head</th>
                      <th className="p-4 text-sm text-gray-500 text-start">Industry Type</th>
                      <th className="p-4 text-sm text-gray-500 text-start">Contact Number</th>
                      <th className="p-4 text-sm text-gray-500 text-start">Number of Employees</th>
                    </tr>
                  </thead>
                  <tbody >
                    {currentUsers.map((user) => (
                      <tr key={user.id} className="border-b hover:bg-gray-100">
                        ...
                        <td className="p-4">{user.name_of_industry}</td>
                        <td className="p-4">{user.name_of_head}</td>
                        <td className="p-4">{user.type}</td>
                        <td className="p-4">{user.phone}</td>
                        <td className="p-4">{user.emp}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
